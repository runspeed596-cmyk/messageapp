...
interface ChatRepository : JpaRepository<Chat, UUID> {
    ...
    @Query("SELECT c FROM Chat c JOIN c.participants p1 JOIN c.participants p2 WHERE c.type = 'PRIVATE' AND p1.id = :userId1 AND p2.id = :userId2")
    fun findPrivateChatBetween(@Param("userId1") userId1: UUID, @Param("userId2") userId2: UUID): Chat?
