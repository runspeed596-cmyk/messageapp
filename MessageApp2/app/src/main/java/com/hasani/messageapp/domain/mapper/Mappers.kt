package com.hasani.messageapp.domain.mapper

import com.hasani.messageapp.data.local.entity.*
import com.hasani.messageapp.data.remote.dto.*
import com.hasani.messageapp.domain.model.*
...
fun GroupMessageEntity.toDomain(replyMessage: Message? = null): Message = Message(
    ...
    poll = if (poll != null) {
        try {
            com.google.gson.Gson().fromJson(poll, com.hasani.messageapp.data.remote.dto.PollDto::class.java)?.toDomain()
        } catch (e: Exception) { null }
    } else null,
    amplitudes = amplitudes?.split(",")?.mapNotNull { it.toIntOrNull() }
)
