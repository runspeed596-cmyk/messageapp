...
import com.Kelasor.app.data.repository.GroupResult
...
import com.Kelasor.app.data.remote.dto.SendGroupMessageRequest
...
class GroupConversationViewModel @Inject constructor(
    ...
    val voiceRecorderManager: com.Kelasor.app.data.voice.VoiceRecorderManager,
    val audioPlayerManager: com.Kelasor.app.data.audio.AudioPlayerManager,
...
class GroupSettingsViewModel @Inject constructor(
     ...
     */
    private fun applyContactNameToUser(user: com.Kelasor.app.domain.model.User): com.Kelasor.app.domain.model.User {
    ...
    fun changeMemberRole(memberId: String, newRole: com.Kelasor.app.domain.model.MemberRole) {
        val groupId = currentGroupId ?: return
        viewModelScope.launch {
...
data class CreateGroupState(
    ...
    val selectedMembers: List<com.Kelasor.app.domain.model.User> = emptyList(),
    val contacts: List<com.Kelasor.app.domain.model.User> = emptyList(),
    val searchQuery: String = "",
    val searchResults: List<com.Kelasor.app.domain.model.User> = emptyList(),
    val isLoading: Boolean = false,
...
class CreateGroupViewModel @Inject constructor(
    ...
    fun addMember(user: com.Kelasor.app.domain.model.User) {
        _state.update { 
...
data class GroupDetailState(
    val group: Group? = null,
    val members: List<GroupMember> = emptyList(),
    val myRole: com.Kelasor.app.domain.model.MemberRole? = null,
...
class GroupDetailViewModel @Inject constructor(
            ...
            groupRepository.getGroupById(groupId).collect { result ->
                when (result) {
                    ...
                    is GroupResult.Success -> {
                        ...
                        val isOwner = group.myRole == com.Kelasor.app.domain.model.MemberRole.OWNER
                        val isAdmin = group.myRole in listOf(
                            com.Kelasor.app.domain.model.MemberRole.OWNER,
                            com.Kelasor.app.domain.model.MemberRole.ADMIN
                        )
    ...
    fun demoteFromAdmin(userId: String) {
        val groupId = currentGroupId ?: return
        viewModelScope.launch {
            groupRepository.changeRole(groupId, userId, com.Kelasor.app.domain.model.MemberRole.MEMBER).collect { result ->
                if (result is GroupResult.Success) {
                     loadGroup(groupId) // Refresh
                } else if (result is GroupResult.Error) {
                    _events.emit(GroupEvent.Error(result.message))
