...
// ═══════════════════════════════════════════════════════════════════════════════
// 📦 Binary Protocol for Android - Mirrors server BinaryProtocol
// Zero-copy deserialization, compact wire format
// ═══════════════════════════════════════════════════════════════════════════════
...
data class Envelope(val opCode: Byte, val payload: ByteArray) {
    ...
    companion object {
        fun deserialize(bytes: ByteArray): Envelope {
...
data class ChatMessageProto(
    ...
    companion object {
        fun deserialize(reader: BinaryReader): ChatMessageProto = ChatMessageProto(
            messageId = reader.readLong(), chatId = reader.readUUID(), senderId = reader.readUUID(),
            messageType = reader.readByte(), content = reader.readString(), mediaId = reader.readString(),
            thumbnailUrl = reader.readString(), replyToId = reader.readLong(),
            timestamp = reader.readLong(), editedAt = reader.readLong()
        )
    }
}

data class ChatListItemProto(
    val chatId: UUID, val chatType: Byte, val name: String, val avatarUrl: String?,
    val lastMessageContent: String?, val lastMessageSenderId: UUID?,
    val lastMessageTime: Long, val unreadCount: Int, val isPinned: Boolean, val isMuted: Boolean
) {
    companion object {
        fun deserialize(reader: BinaryReader): ChatListItemProto {
            val chatId = reader.readUUID(); val chatType = reader.readByte()
            val name = reader.readString()!!; val avatarUrl = reader.readString()
            ...
            val lastMessageTime = reader.readLong(); val unreadCount = reader.readInt()
            val isPinned = reader.readByte() == 1.toByte(); val isMuted = reader.readByte() == 1.toByte()
...
data class TypingEventProto(val chatId: UUID, val userId: UUID, val isTyping: Boolean) {
    fun serialize(writer: BinaryWriter): BinaryWriter {
        writer.writeUUID(chatId); writer.writeUUID(userId); writer.writeByte(if (isTyping) 1 else 0)
        return writer
    }
    companion object {
        fun deserialize(reader: BinaryReader): TypingEventProto = TypingEventProto(
            chatId = reader.readUUID(), userId = reader.readUUID(), isTyping = reader.readByte() == 1.toByte()
        )
    }
}

data class ReadReceiptProto(val chatId: UUID, val userId: UUID, val lastReadMessageId: Long, val timestamp: Long) {
    fun serialize(writer: BinaryWriter): BinaryWriter {
        writer.writeUUID(chatId); writer.writeUUID(userId); writer.writeLong(lastReadMessageId); writer.writeLong(timestamp)
        return writer
    }
    companion object {
        fun deserialize(reader: BinaryReader): ReadReceiptProto = ReadReceiptProto(
            chatId = reader.readUUID(), userId = reader.readUUID(),
            lastReadMessageId = reader.readLong(), timestamp = reader.readLong()
        )
    }
}

data class StoryItemProto(
    val storyId: Long, val userId: UUID, val mediaType: Byte, val mediaUrl: String?,
    val thumbnailUrl: String?, val caption: String?, val timestamp: Long, val expiresAt: Long, val viewCount: Int
) {
    companion object {
        fun deserialize(reader: BinaryReader): StoryItemProto = StoryItemProto(
            storyId = reader.readLong(), userId = reader.readUUID(), mediaType = reader.readByte(),
            mediaUrl = reader.readString(), thumbnailUrl = reader.readString(), caption = reader.readString(),
            timestamp = reader.readLong(), expiresAt = reader.readLong(), viewCount = reader.readInt()
        )
    }
}

data class CursorPageRequest(val chatId: UUID?, val cursorId: Long, val limit: Int) {
    fun serialize(writer: BinaryWriter): BinaryWriter {
        if (chatId != null) { writer.writeByte(1); writer.writeUUID(chatId) } else { writer.writeByte(0) }
        writer.writeLong(cursorId); writer.writeInt(limit)
        return writer
    }
}

data class AuthRequestProto(val token: String) {
    fun serialize(writer: BinaryWriter): BinaryWriter { writer.writeString(token); return writer }
}

data class AuthResponseProto(val isSuccess: Boolean, val userId: UUID?, val errorMessage: String?) {
    companion object {
        fun deserialize(reader: BinaryReader): AuthResponseProto {
            ...
            return AuthResponseProto(isSuccess, userId, reader.readString())
        }
    }
}
