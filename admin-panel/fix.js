...
function applyPagination(arrayName) {
    const regex = new RegExp(arrayName + '\\n\\s*\\.sort\\(\\(a, b\\) => a\\.displayOrder - b\\.displayOrder\\)\\n\\s*\\.map\\(\\(', 'g');
    content = content.replace(regex, arrayName + '\n                                    .sort((a, b) => a.displayOrder - b.displayOrder)\n                                    .slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE)\n                                    .map((');
}

applyPagination('fieldsOfStudy');
applyPagination('faculties');
applyPagination('educationLevels');
applyPagination('educationalRoles');
applyPagination('clubs');
applyPagination('studentOrgs');

// For slider banners
content = content.replace(/sliderBanners\\n\\s*\\.sort\\(\\(a, b\\) => a\\.displayOrder - b\\.displayOrder\\)\\n\\s*\\.map\\(\\(/g, 'sliderBanners\n                                    .sort((a, b) => a.displayOrder - b.displayOrder)\n                                    .slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE)\n                                    .map((');
